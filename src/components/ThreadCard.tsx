...
export default function ThreadCard({
  ...
  return (
    <article className="bg-surface-container-lowest border-2 border-outline-variant hover:border-primary hover:scale-[1.015] hover:shadow-[0_6px_16px_rgba(0,0,0,0.06)] dark:hover:shadow-[0_0_20px_1px_rgba(255,255,255,0.12)] rounded-xl p-6 transition-all duration-150 ease-out cursor-pointer group flex flex-col justify-between min-h-[260px] md:min-h-[220px] outline-none focus-within:ring-4 focus-within:ring-primary/20">
      <div className="flex flex-col md:flex-row justify-between items-start gap-4">
        {/* Main Details */}
        <div className="flex-1">
          <div className="flex flex-wrap items-center gap-3 mb-3">
            <span className="px-3 py-1.5 bg-surface-container-high border border-outline-variant text-label-md font-bold text-on-surface-variant rounded-lg">
              {category}
            </span>
          ...
          <h4 className="font-headline-md text-headline-md text-on-surface group-hover:text-primary transition-colors mb-3 font-extrabold line-clamp-2">
            {title}
          </h4>
        ...
        {/* Replies Widget */}
        <div 
          className="flex flex-row md:flex-col items-center justify-center gap-2 bg-surface-container border border-outline-variant p-4 rounded-xl min-w-[100px] w-full md:w-auto h-auto md:h-28 self-stretch md:self-auto"
